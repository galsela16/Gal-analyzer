...
function configureTfFft(n){
  ...
  tfPxx=new Float32Array(TF_FFT_N/2); tfPyy=new Float32Array(TF_FFT_N/2);
...
function buildBands(bpo){
  ...
  avgBuf=new Array(BANDS).fill(NaN);
...
safeOn('smooth', 'input',e=>{
  ...
  // RTA smoothing is applied once, in linear power, with a real-time constant.
  // Keep Web Audio smoothing disabled so it is not applied a second time.
  if(analyser) analyser.smoothingTimeConstant=0;
...
safeOn('avgBtn', 'click',function(){
  avgOn=!avgOn; this.classList.toggle('on',avgOn); avgBuf.fill(NaN); _lastRtaSmoothTime=0;
});
...
function exportSessionJson(){
  const data = {
    version: 'v5.5.77-delay-field-rebuild',
    ...
    settings: {
      calib, floorDb, curBpo, fftSize, targetMode, targetVisible, weightMode, sunMode, eqMinFreq, eqMaxFreq
    }
  };
...
function eqRtaRangeX(f){
  const lo=ISO[0],hi=ISO[BANDS-1],W=cv.clientWidth;
  return Math.log(Math.max(lo,Math.min(hi,f))/lo)/Math.log(hi/lo)*W;
}
function eqRtaRangeFreq(px){
...
cv.addEventListener('pointerleave',()=>{ cursorX=null;if(!eqRtaRangeDrag)cv.style.cursor=''; });
cv.addEventListener('pointerup',e=>{
  if(eqRtaRangeDrag){eqRtaRangeDrag=null;setEqCorrectionRange(eqMinFreq,eqMaxFreq,true);return;}
  if(!dragging) return; dragging=false;
  if(Math.abs(dragX1-dragX0)>20) applyZoom(dragX0,dragX1);
});
cv.addEventListener('pointercancel',()=>{dragging=false;eqRtaRangeDrag=null;});
...
function abortRT60(){
  ...
  if(rt60ArmTimer){clearTimeout(rt60ArmTimer);rt60ArmTimer=null;}
...
safeOn('tfSwapBtn', 'click',function(){ tfSwap=!tfSwap; resetTfAutoDelay(); resetDistanceCalibration(); this.classList.toggle('on',tfSwap); });
...
function tfBandCoherence(fc,halfWidthRatio,sr){
  const lo=fc/halfWidthRatio,hi=fc*halfWidthRatio,n=tfPxx.length;
  const first=Math.max(1,Math.floor(lo/sr*TF_FFT_N)),last=Math.min(n-1,Math.ceil(hi/sr*TF_FFT_N));
  let weighted=0,weight=0;
  for(let k=first;k<=last;k++){
    const w=tfPxx[k]*tfPyy[k];if(w<=1e-20)continue;
    weighted+=tfPxyRe[k]*tfPxyRe[k]+tfPxyIm[k]*tfPxyIm[k];weight+=w;
  }
  return Math.max(0,Math.min(1,weighted/(weight+1e-20)));
}
function tfCompute(){
  if(!tfFrames){ return; }
  const bins=tfMic.length, nyq=audioCtx.sampleRate/2, R6=Math.pow(2,1/6);
  const H=[], refB=[],coh=[]; let refMax=-999;
  for(let k=0;k<GEQ.length;k++){
    const fc=GEQ[k];
    ...
    H[k]=micDb-refB[k];
    coh[k]=tfBandCoherence(fc,R6,audioCtx.sampleRate);
    refMax=Math.max(refMax,refB[k]);
  }
  if(micCal){ for(let k=0;k<GEQ.length;k++) H[k]-=micCalAt(GEQ[k]); }
  const rel=GEQ.map((f,k)=> refB[k]>refMax-25 && coh[k]>=tfCohGate && f>=eqMinFreq && f<=eqMaxFreq);
...
function renderDlySpk(){
  ...
  box.querySelectorAll('.dlyMeasOne').forEach(b=>b.addEventListener('click',function(){
    ...
    pickSource(()=>runDelayCapture(btn,(res,silent)=>{
      ...
      renderDlySpk();
    },{maxDelayMs:delaySearchMs,mode:'speaker',repeatabilityKey:'speaker:'+i}),3800);
  }));
}
...
function startRT60(){
  ...
  rt60ArmTimer=setTimeout(()=>{
    ...
    rt60CutTimer=setTimeout(()=>{
      rt60CutTimer=null;if(rt60State!=='capture')return;
      const t=audioCtx.currentTime;
      ...
      rt60FinishTimer=setTimeout(()=>{
        rt60FinishTimer=null;if(rt60State!=='capture')return;
        rt60State='idle'; 
        if(rt60Timer){ clearInterval(rt60Timer); rt60Timer=null; }
...
function binOverlapPowerDb(data,fLo,fHi,nyquist){
  const bins=data.length, binHz=nyquist/bins;
  const lo=Math.max(0,fLo), hi=Math.min(nyquist,fHi);
  if(!(hi>lo)) return -120;
  const first=Math.max(0,Math.floor(lo/binHz-0.5));
  const last=Math.min(bins-1,Math.ceil(hi/binHz-0.5));
  let power=0;
  for(let i=first;i<=last;i++){
    const binLo=Math.max(0,(i-0.5)*binHz), binHi=Math.min(nyquist,(i+0.5)*binHz);
...
function smoothBandDb(index,nextDb,alpha){
  ...
  const power=Number.isFinite(previous)?previous*alpha+nextPower*(1-alpha):nextPower;
  avgBuf[index]=power;
  return 10*Math.log10(power+1e-12);
}
function heat(t){
...
function computeComplexTf(){
  ...
  for(let k=0; k<halfN; k++){
    ...
    const rawRe = ry*rx + iy*ix;
    const rawIm = iy*rx - ry*ix;
...
function drawRta(W,H,nyquist,bins,xForFreq){
  ...
  for(let b=0;b<BANDS;b++){
    const fc=ISO[b];
    const rawDb=calibratedResponseDb(binOverlapPowerDb(floatData,fc/R,fc*R,nyquist),fc);
    const displayDb=smoothBandDb(b,rawDb,smoothAlpha);
  ...
  if(tfRequested){
    ...
    if(!frozen) analyserRef.getFloatFrequencyData(floatDataRef);
    for(let b=0;b<BANDS;b++){
      const rv=norm(binOverlapPowerDb(floatDataRef,ISO[b]/R,ISO[b]*R,nyquist));
...
const wfResonance={enabled:true,frame:0,started:0,tracks:new Map(),candidates:[],maxHz:20000,minPromDb:3,minAgeMs:2400};
...
function spectrumPeakDetail(data,centerHz,nyquist){
...
function updateWaterfallResonances(nyquist){
  ...
  const ranked=[...wfResonance.tracks.values()].filter(t=>t.total>=14&&t.score>=.58&&(t.prom>=wfResonance.minPromDb||t.narrowProm>=7))
    .sort((a,b)=>(b.score*(b.prom+.55*(b.narrowProm||0)))-(a.score*(a.prom+.55*(a.narrowProm||0))));
  const chosen=[];
  for(const t of ranked){if(chosen.some(c=>Math.abs(Math.log2(c.hz/t.hz))<.12))continue;chosen.push(t);if(chosen.length>=3)break;}
...
function drawWaterfallResonanceOverlay(W,specH,xForFreq){
  ...
  ctx.save();
  const visible=wfResonance.candidates.filter(r=>{
  ...
  if(metric&&mode==='spec')metric.textContent=visible.length
    ?'Resonance · '+visible.slice(0,2).map(r=>Math.round(r.hz)+'Hz').join(' · ')
...
function drawWaterfall3d(W,H,nyquist,xForFreq){
  ...
  for(let j=0;j<=4;j++){
    const p=j/4,y=base-p*amp,label=Math.round(floorDb+(ceilDb-floorDb)*p);
    ctx.fillStyle=sunMode?'rgba(71,85,105,.72)':'rgba(147,163,173,.78)';ctx.fillText(String(label),7,Math.max(24,y+3));
  }
...
function drawSpec(W,H,nyquist,bins,xForFreq){
  ...
  for(let px=0;px<specCanvas.width;px++){
    const x0=Math.max(0,px-.5)/specCanvas.width, x1=Math.min(specCanvas.width,px+.5)/specCanvas.width;
    const fLo=Math.exp(x0*(Math.log(ISO[BANDS-1])-Math.log(ISO[0]))+Math.log(ISO[0]));
    const fHi=Math.exp(x1*(Math.log(ISO[BANDS-1])-Math.log(ISO[0]))+Math.log(ISO[0]));
    const fc=Math.sqrt(fLo*fHi);
    const db=calibratedResponseDb(binOverlapPowerDb(floatData,fLo,fHi,nyquist),fc);
...
(function initPrefs(){
  ...
  resetTfAutoDelay();
  const ver=document.getElementById('ver'); if(ver) ver.textContent='V5.5.77';
  v3UpdateStatus();
})();
(function initAccent(){
...
function v52Init(){
  ...
  window.v53ToggleMicCal=function(){
    if(!micArea)return;
    const open=!micArea.classList.contains('open');
    micArea.classList.toggle('open',open);
    const button=document.getElementById('v52OpenMicCal');if(button)button.classList.toggle('on',open);
    if(open){renderCalList();requestAnimationFrame(()=>{micArea.scrollIntoView({block:'nearest',behavior:'smooth'});const d=document.getElementById('v52IODock'),s=document.getElementById('stage');if(d&&s)s.style.setProperty('--v52-io-h',Math.ceil(d.getBoundingClientRect().height)+'px');resize();});}
